...
def _get_team_season_stats_leaders(is_season_specific: bool, is_users_only: bool):
    ...
    if is_season_specific and not is_users_only:
        ...
        converted_total_points: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in total_points_data]
        converted_total_yards: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in total_yards_data]
        converted_total_ypg: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in total_ypg_data]
        converted_off_yards: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in off_yards_data]
        converted_off_ypg: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in off_ypg_data]
        converted_ppg: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in ppg_data]
        converted_pass_yds: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in pass_yds_data]
        converted_pass_tds: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in pass_tds_data]
        converted_ints: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in ints_data]
        converted_sacked: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in sacked_data]
        converted_rush_yds: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in rush_yds_data]
        converted_rush_tds: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in rush_tds_data]
        converted_fumbles: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in fumbles_data]
        converted_drops: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in drops_data]
        converted_off_turnovers: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in off_turnovers_data]
        # Defense
        converted_sacks: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in sacks_data]
        converted_tfl: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in tfl_data]
        converted_ints_made: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in ints_made_data]
        converted_ff: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in ff_data]
        converted_fr: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in fr_data]
        converted_def_turnovers: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in def_turnovers_data]
        converted_pass_def: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in pass_def_data]
        converted_safeties: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in safeties_data]
        converted_blocked_kicks: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in blocked_kicks_data]
        converted_def_tds: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in def_tds_data]
        converted_to_margin: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in to_margin_data]
        # Special Teams
        converted_kr_yds: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in kr_yds_data]
        converted_kr_tds: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in kr_tds_data]
        converted_pr_yds: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in pr_yds_data]
        converted_pr_tds: List[TeamSeasonRecord] = [
            _get_team_info_and_season_stats(team) for team in pr_tds_data]
