...
class CoachsService():
    ...
    def get_coach_records(self) -> dict:
        ...
        for user in users:
            user_coachs_info: List[CoachInfoData] = self.CoachsDataService.get_coach_info(user.id)
            user_coachs_records: List[CoachInfoData] = self.CoachsDataService.get_coach_records(user.id)
            response[user.id] = self._get_coach_season_records(user_coachs_info, user_coachs_records)

        return response

    def _get_coach_season_records(self, coach_info: List[CoachInfoData], coach_stats: List[CoachStatsData]) -> dict:
        coach_data = {}
        ...
        coach_data['titles'] = sum(
            [1 for year in coach_stats if year.national_title])
