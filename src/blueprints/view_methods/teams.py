...
def get_team_player_stats(request, team_id):
    ...
    converted_kicking_stats: List[PlayerKickingStats] = [_get_player_kicking_stats(player) for player in kicking_stats]
    converted_players_return: List[PlayerReturnStats] = [_get_player_return_stats(player) for player in return_stats]
    ...
    kick_return_stats = sorted(converted_players_return, key=lambda p: p.return_stats.kr_yds, reverse=True)
    punt_return_stats = sorted(converted_players_return, key=lambda p: p.return_stats.pr_yds, reverse=True)
    kick_stats = sorted(converted_kicking_stats, key=lambda p: p.kicking_stats.fg_made, reverse=True)
    punt_stats = sorted(converted_kicking_stats, key=lambda p: p.kicking_stats.number_punts, reverse=True)
    # Stats leaders
    ...
    kick_return_leaders = sorted(converted_players_return, key=lambda p: p.return_stats.kr_yds, reverse=True)[:3]
    kicking_leaders = sorted(converted_kicking_stats, key=lambda p: p.kicking_stats.fg_made, reverse=True)[:3]
    punt_return_leaders = sorted(converted_players_return, key=lambda p: p.return_stats.pr_yds, reverse=True)[:3]
    punting_leaders = sorted(converted_kicking_stats, key=lambda p: p.kicking_stats.number_punts, reverse=True)[:3]
    ...
    passing_stats_json = defensive_stats_schema.dump(passing_stats)
    punting_stats_json = kicking_stats_schema.dump(punt_stats)
    punt_return_json = return_stats_schema.dump(punt_return_stats)
    receiving_stats_json = defensive_stats_schema.dump(receiving_stats)
    rushing_stats_json = defensive_stats_schema.dump(rushing_stats)
    # Stats leaders
