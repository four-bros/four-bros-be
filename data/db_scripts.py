...
def insert_def_stats_into_db(def_stats):
    ...
    for i, value in enumerate(def_stats):
        ...
        player: DefensiveStatsData = session.query(DefensiveStatsData).where(
            DefensiveStatsData.id == new_player.id).scalar()

        if not player or (player and player.year != current_year):
            session.add(new_player)
            session.flush()
        else:
            update(DefensiveStatsData).where(DefensiveStatsData.id == new_player.id).values(
...
def insert_kicking_stats_into_db(kicking_stats):
    ...
    for i, value in enumerate(kicking_stats):
        ...
        player: KickingStatsData = session.query(KickingStatsData).where(
            KickingStatsData.id == new_player.id
        ).scalar()
        
        if not player or (player.year != current_year):
            session.add(new_player)
            session.flush()
        else:
...
def insert_off_stats_into_db(off_stats):
    ...
    for i, value in enumerate(off_stats):
        ...
        player: OffensiveStatsData = session.query(OffensiveStatsData).where(
            OffensiveStatsData.id == new_player.id).scalar()
...
def insert_return_stats_into_db(return_stats):
    ...
    for i, value in enumerate(return_stats):
        ...
        player: ReturnStatsData = session.query(ReturnStatsData).where(
            ReturnStatsData.id == new_player.id).scalar()
